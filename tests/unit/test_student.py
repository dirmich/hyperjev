...
class StudentContractTests(unittest.TestCase):
    ...
    def test_reference_char_bow_encoder_is_a_compatible_backbone(self) -> None:
        ...
        model = build_torch_model(
            self.registry,
            StudentConfig(backbone="reference-char-bow-encoder", precision="fp32"),
        )
        output = model(
            "memory.type",
            torch.tensor([[2, 17, 0, 0]], dtype=torch.long),
    ...
    def test_reference_hybrid_bow_encoder_is_a_compatible_backbone(self) -> None:
        try:
            import torch
        except ImportError:
            self.skipTest("PyTorch is optional")
        model = build_torch_model(
            self.registry,
            StudentConfig(backbone="reference-hybrid-bow-encoder", precision="fp32"),
        )
        output = model(
            "memory.type",
            torch.tensor([[2, 17, 16384, 17001]], dtype=torch.long),
            torch.tensor([[1, 1, 1, 1]], dtype=torch.long),
        )
        self.assertTrue(model.use_bow_encoder)
        self.assertEqual(output["type"], "choice")

    def test_reference_control_bow_encoder_is_a_compatible_backbone(self) -> None:
        try:
            import torch
        except ImportError:
            self.skipTest("PyTorch is optional")
    ...
    def test_reference_segmented_bow_encoder_is_a_compatible_backbone(self) -> None:
        try:
            import torch
        except ImportError:
            self.skipTest("PyTorch is optional")
        model = build_torch_model(
            self.registry,
            StudentConfig(backbone="reference-segmented-bow-encoder", precision="fp32"),
        )
        output = model(
            "memory.type",
            torch.tensor([[2, 17, 16384, 17001]], dtype=torch.long),
